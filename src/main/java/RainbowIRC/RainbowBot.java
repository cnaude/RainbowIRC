...
import RainbowIRC.IRCListeners.ActionListener;
import RainbowIRC.IRCListeners.ConnectListener;
import RainbowIRC.IRCListeners.DisconnectListener;
import RainbowIRC.IRCListeners.JoinListener;
import RainbowIRC.IRCListeners.KickListener;
import RainbowIRC.IRCListeners.MessageListener;
import RainbowIRC.IRCListeners.ModeListener;
import RainbowIRC.IRCListeners.MotdListener;
import RainbowIRC.IRCListeners.NickChangeListener;
import RainbowIRC.IRCListeners.NoticeListener;
import RainbowIRC.IRCListeners.PartListener;
import RainbowIRC.IRCListeners.PrivateMessageListener;
import RainbowIRC.IRCListeners.QuitListener;
import RainbowIRC.IRCListeners.ServerResponseListener;
import RainbowIRC.IRCListeners.TopicListener;
import RainbowIRC.IRCListeners.VersionListener;
import RainbowIRC.IRCListeners.WhoisListener;
 ...
 * @author Chris Naude
 */
public final class RainbowBot {

    private PircBotX bot;

    public final MyPlugin plugin;
    private final File file;
    private RainbowIRC.Configuration.Configuration config;
    ...
    public ArrayList<MC_Player> whoisSenders;
     ...
     * @param plugin
     */
    public RainbowBot(File file, MyPlugin plugin) {
    ...
    public void reload(MC_Player sender) {
        plugin.sendMessageToSender(sender, "Reloading bot: " + botNick);
        reload();
    }
    ...
    public void reloadConfig(MC_Player sender) {
        loadConfig();
        plugin.sendMessageToSender(sender, plugin.LOG_HEADER_F + " [" + botNick + "] IRC bot configuration reloaded.");
    }
    ...
    public void mute(String channelName, MC_Player sender, String user) {
        if (muteList.get(channelName).contains(user)) {
            plugin.sendMessageToSender(sender, "User '" + user + "' is already muted.");
        } else {
            plugin.sendMessageToSender(sender, "User '" + user + "' is now muted.");
    ...
    public void muteList(String channelName, MC_Player sender) {
        if (muteList.get(channelName).isEmpty()) {
            plugin.sendMessageToSender(sender, "There are no users muted for " + channelName);
        } else {
            plugin.sendMessageToSender(sender, "Muted users for " + channelName
    ...
    public void unMute(String channelName, MC_Player sender, String user) {
        if (muteList.get(channelName).contains(user)) {
            plugin.sendMessageToSender(sender, "User '" + user + "' is no longer muted.");
            muteList.get(channelName).remove(user);

        } else {
            plugin.sendMessageToSender(sender, "User '" + user + "' is not muted.");
        }
    }

    public void asyncConnect(MC_Player sender) {
        plugin.sendMessageToSender(sender, connectMessage);
        asyncConnect();
    }
    ...
    public void blockingIRCMessage(final String target, final String message) {
        ...
        plugin.logDebug("[blockingIRCMessage] About to send IRC message: " + message);
    ...
    public void changeLogin(MC_Player sender, String newLogin) {
        plugin.sendMessageToSender(sender, ChatColor.DARK_PURPLE
    ...
    private void loadConfig() {
        try {
            ...
            if (botRealName.isEmpty()) {
                botRealName = plugin.getDescription().getName();
            ...
            quitMessage = plugin.colorConverter.translateAlternateColorCodes('&', config.getString("quit-message", ""));
    ...
    private boolean isPlayerInValidWorld(MC_Player player, String channelName) {
     ...
     * @param message
     */
    public void gameChat(MC_Player player, String message) {
        if (!this.isConnected()) {
            return;
        }
        for (String channelName : botChannels) {
            ...
            if (isMessageEnabled(channelName, TemplateName.GAME_CHAT)) {
                plugin.logDebug("[" + TemplateName.GAME_CHAT + "] => " + channelName + " => " + message);
                asyncIRCMessage(channelName, plugin.tokenizer
                        .gameChatToIRCTokenizer(player, plugin.getMsgTemplate(botNick, TemplateName.GAME_CHAT), message));
            } else {
                plugin.logDebug("Ignoring message due to " + TemplateName.GAME_CHAT + " not being listed.");
            }
        }
    }
     ...
     * @param message
     */
    public void gameChat(MC_Player player, String channelName, String message) {
        if (!this.isConnected()) {
            return;
        }
        if (isValidChannel(channelName)) {
     ...
     * @param message
     */
    public void gameBroadcast(MC_Player player, String message) {
        if (!this.isConnected()) {
            return;
        }
        for (String channelName : botChannels) {
    ...
    public void consoleBroadcast(String message) {
        ...
        for (String channelName : botChannels) {
            if (isMessageEnabled(channelName, TemplateName.BROADCAST_CONSOLE_MESSAGE)) {
                asyncIRCMessage(channelName, plugin.tokenizer
                        .gameChatToIRCTokenizer(plugin.getMsgTemplate(botNick,
                                        TemplateName.BROADCAST_CONSOLE_MESSAGE),
                                plugin.colorConverter.translateAlternateColorCodes('&', message)));
            }
        }
    }
     ...
     * @param message
     */
    public void gameJoin(String playerName, String message) {
        ...
        for (String channelName : botChannels) {
            if (isMessageEnabled(channelName, TemplateName.GAME_JOIN)) {
                ...
                if (player == null) {
                    asyncIRCMessage(channelName, plugin.tokenizer
                            .gameChatToIRCTokenizer(playerName, plugin.getMsgTemplate(
     ...
     * @param message
     */
    public void gameServerSwitch(MC_Player player, String message) {
        if (!this.isConnected()) {
            return;
        }
        for (String channelName : botChannels) {
     ...
     * @param message
     */
    public void gameQuit(MC_Player player, String message) {
        if (!this.isConnected()) {
            return;
        }
        for (String channelName : botChannels) {
     ...
     * @param reason
     */
    public void gameKick(MC_Player player, String message, String reason) {
        if (!this.isConnected()) {
            return;
        }
        for (String channelName : botChannels) {
     ...
     * @param message
     */
    public void gameAction(MC_Player player, String message) {
        if (!this.isConnected()) {
            return;
        }
        for (String channelName : botChannels) {
     ...
     * @param templateName
     */
    public void gameDeath(MC_Player player, String message, String templateName) {
        if (!this.isConnected()) {
            return;
        }
        for (String channelName : botChannels) {
     ...
     * @param sender
     */
    public void changeTopic(String channelName, String topic, MC_Player sender) {
        ...
        if (channel != null) {
            ...
            plugin.sendMessageToSender(sender, "IRC topic for " + channelName + " changed to \"" + topic + "\"");
        } else {
            plugin.sendMessageToSender(sender, "Invalid channel: " + channelName);
        }
    }
     ...
     * @param botServer
     */
    public void setServer(MC_Player sender, String botServer) {
     ...
     * @param auto
     */
    public void setServer(MC_Player sender, String server, Boolean auto) {
        ...
        plugin.sendMessageToSender(sender, "IRC server changed to \"" + botServer + ":"
    ...
    public void addOp(String channelName, String userMask, MC_Player sender) {
        if (opsList.get(channelName).contains(userMask)) {
            plugin.sendMessageToSender(sender, "User mask " + ChatColor.WHITE + userMask
    ...
    public void asyncQuit(MC_Player sender) {
        plugin.sendMessageToSender(sender, "Disconnecting " + bot.getNick() + " from IRC server " + botServer);
        asyncQuit(false);
    }
     ...
     * @param nick
     */
    public void sendUserWhois(MC_Player sender, String nick) {
        User user = null;
        ...
        if (user == null) {
            plugin.sendMessageToSender(sender, ChatColor.RED + "Invalid user: " + ChatColor.WHITE + nick);
        } else {
     ...
     * @param channelName
     */
    public void sendUserList(MC_Player sender, String channelName) {
        String invalidChannel = ChatColor.RED + "Invalid channel: "
                + ChatColor.WHITE + channelName;
        if (!isValidChannel(channelName)) {
            plugin.sendMessageToSender(sender, invalidChannel);
            return;
        }
    ...
    public void sendUserList(MC_Player sender, Channel channel) {
        String channelName = channel.getName();
        if (!isValidChannel(channelName)) {
            plugin.sendMessageToSender(sender, ChatColor.RED + "Invalid channel: "
                    + ChatColor.WHITE + channelName);
            return;
        }
        ...
        for (String userName : channelUsers) {
            plugin.sendMessageToSender(sender, "  " + ChatColor.WHITE + userName);
        }
    }
     ...
     * @param sender
     */
    public void sendUserList(MC_Player sender) {
        for (Channel channel : getChannels()) {
    ...
    public void broadcastChat(User user, org.pircbotx.Channel channel, String target, String message, boolean override, boolean ctcpResponse) {
        ...
        if (enabledMessages.get(myChannel).contains(TemplateName.IRC_CHAT) || override) {
            ...
            if (!newMessage.isEmpty()) {
                plugin.broadcastMessage(newMessage, "irc.message.chat");
                messageSent = true;
            }
        } else {
        ...
        if (enabledMessages.get(myChannel).contains(TemplateName.IRC_CONSOLE_CHAT)) {
            ...
            plugin.getServer().broadcastMessage(plugin.tokenizer.ircChatToGameTokenizer(
    ...
    public void playerChat(User user, org.pircbotx.Channel channel, String target, String message) {
        ...
        if (message.contains(" ")) {
            ...
            if (enabledMessages.get(myChannel).contains(TemplateName.IRC_PCHAT)) {
                ...
                MC_Player player = plugin.getPlayer(pName);
                if (player != null) {
...
// Broadcast action messages from IRC
     ...
     * @param params
     */
    public void commandNotify(MC_Player player, String cmd, String params) {
        if (!this.isConnected()) {
            return;
        }
     ...
     * @param message
     */
    public void msgPlayer(MC_Player sender, String nick, String message) {
     ...
     * @param newNick
     */
    public void asyncChangeNick(final MC_Player sender, final String newNick) {
        if (!this.isConnected()) {
            return;
        }
        new Thread(new Runnable() {
        ...
        plugin.sendMessageToSender(sender, "Setting nickname to " + newNick);
    ...
    public void saveConfig(MC_Player sender) {
        try {
            ...
            plugin.sendMessageToSender(sender, plugin.LOG_HEADER_F + " Saving bot \"" + botNick + "\" to " + file.getName());
        } catch (IOException ex) {
            plugin.logError(ex.getMessage());
            plugin.sendMessageToSender(sender, ex.getMessage());
        }
    }
